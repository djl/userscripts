...
const boxes = {
    cd: {
        'orpheus.network': ['format_1', 'bitrate_0', 'media_0', 'needlog', 'needcksum', 'needcue'],
        'redacted.ch': ['format_1', 'bitrate_8', 'media_0', 'needlog', 'needcue', 'needchecksum'],
    },
    web: {
    ...
    cdweb: {
        'orpheus.network': ['format_1', 'bitrate_0', 'media_0', 'media_1', 'needlog', 'needcksum', 'needcue'],
        'redacted.ch': ['format_1', 'bitrate_8', 'media_0', 'media_7', 'needlog', 'needcue', 'needchecksum'],
    },
    cdweb24bit: {
        'orpheus.network': ['format_1', 'bitrate_0', 'bitrate_1', 'media_0', 'media_1', 'needlog', 'needcksum', 'needcue'],
        'redacted.ch': ['format_1', 'bitrate_8', 'bitrate_9', 'media_0', 'media_7', 'needlog', 'needcue', 'needchecksum'],
    },
};
...
function clear() {
    ...
    elements.forEach(function (elem) {
        document.querySelectorAll(`tr#${elem} input[type=checkbox]`).forEach(function (box) {
            box.checked = false;
        });
    });
...
function createButton(text, key) {
    ...
    button.onclick = function () {
        ...
        boxes[key][window.location.hostname].forEach(function (box) {
            document.querySelectorAll(`input#${box}[type=checkbox]`).forEach(function (box) {
                box.click();
            });
        });
